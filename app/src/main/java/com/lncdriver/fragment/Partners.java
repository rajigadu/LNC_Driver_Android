...
public class Partners extends Fragment {
    ...
    public static void MyrequestsListRequest() {
        ...
        Utils.global.mapMain.put(ConstVariable.URL, com.lncdriver.utils.Settings.URL_MANAGE_PARTNERS);

        if (Utils.isNetworkAvailable(mcontext)) {
            JsonPost.getNetworkResponse(mcontext, null, Utils.global.mapMain, ConstVariable.PartnersList);
        } else {
    ...
    public static void partnerOnOffListRequest(HashMap<String, Object> map, String type) {
        ...
        Utils.global.mapMain();
        Utils.global.mapMain.put("driverid", id);
        Utils.global.mapMain.put("partner_id", map.get("id").toString());
        Utils.global.mapMain.put("status", type);

        Utils.global.mapMain.put(ConstVariable.URL, com.lncdriver.utils.Settings.URL_PARTNERACTIVATE);

        if (Utils.isNetworkAvailable(mcontext)) {
            JsonPost.getNetworkResponse(mcontext, null, Utils.global.mapMain, ConstVariable.ActivatePartner);
        } else {
            Utils.showInternetErrorMessage(mcontext);
        }
    }
