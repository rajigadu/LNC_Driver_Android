...
public class Forgot extends AppCompatActivity {
    ...
    public void submit() {
        ...
        if (email.getText().toString().trim().isEmpty()) {
            ...
            Utils.global.mapMain.put("email", email.getText().toString().trim());
            Utils.global.mapMain.put(ConstVariable.URL, Settings.URL_FORGETPASSWORD);

            if (Utils.isNetworkAvailable(Forgot.this)) {
                JsonPost.getNetworkResponse(Forgot.this, null, Utils.global.mapMain, ConstVariable.ForgetPassword);
            } else {
